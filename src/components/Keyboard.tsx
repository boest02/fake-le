...
function Keyboard({used, matched}: KeyboardProps) {

  const checkLetter = (letter: string) => {
    if(letter.length > 1) return '' //skip

    if(matched.exact.includes(letter)) return 'exact'
    if(matched.almost.includes(letter)) return 'almost'
    if(used.includes(letter)) return 'used'
          ...
          <div key={row} className={`row row-${row}`}>
            {
              (row < 2) ?
                keyboard.map((letter, key) =>
                  <div className={checkLetter(letter)} role="button" tabIndex={0} key={key} data-action={letter}>
                    {letter}
                  </div>)
                :
                <>
